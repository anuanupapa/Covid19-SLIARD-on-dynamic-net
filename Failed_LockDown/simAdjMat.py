...
levs=[3,4,5,6,2]
AM=construct_net(levs, ld=1)
S_ini = (3*4*5*6*2)-1#int(input('S initial : '))
...
for i in range(N):
  ...
  if TotI[i]>=InfThresh and iterLD%1500==0:
  ...
  elif TotI[i]<InfThresh and iterNLD%300==0:
    AM=construct_net(levs,1)
    iterLD=0
    LDCounter=0
    print('Normal')
    print(TotS[i],TotL[i],TotI[i],TotA[i],TotR[i],TotD[i])
...
plt.savefig('multifail1/All'+'.png')
plt.show()
...
plt.legend()
plt.savefig('multifail1/Infected.png')
plt.show()
...
plt.ylabel('Number of indivduals')
plt.legend()
plt.savefig('multifail1/Ass.png')
plt.show()
...
plt.ylabel('Number of indivduals')
plt.legend()
plt.savefig('multifail1/Dead.png')
plt.show()
